...
\usepackage{xcolor}
\definecolor{kispiblue}{HTML}{02285E}
\definecolor{kispired}{HTML}{FF0000}
\definecolor{kispilink}{HTML}{0068FF}
 \color{kispiblue} %default
...
\affil[1]{Department of Intensive Care and Neonatology, University Children's Hospital Zurich, University of Zurich. {\color{kispilink}Dylan.Lawless@uzh.ch}}
...
\begin{center}
    \includegraphics[scale=0.35]{../output/aggregate_variants_per_criteria}
...
%\texttt{<SAMPLE\_ID>\_<NGS\_ID>\_<POOL\_ID>\_<S\#>\_<LANE>\_<R1|R2>.fastq.gz}
% and Illumina fastq header:
%\texttt{\@<instrument>:<run number>:<flowcell ID>:<lane>:<tile>:<x-pos>:<y-pos> <read>:<is filtered>:<control number>:<sample number>}
